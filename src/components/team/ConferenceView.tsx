  ...
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg p-4">
      ...
      <div className="text-center mb-4">
        <span className="font-headline text-2xl tabular-nums text-[var(--text-primary)]">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div>
          ...
          <p className="font-headline text-lg tabular-nums text-[var(--text-primary)]">{data.points_per_game.toFixed(1)}</p>
        </div>
        <div>
          <p className="text-[var(--text-muted)]">Points Allowed</p>
          <p className="font-headline text-lg tabular-nums text-[var(--text-primary)]">{data.points_allowed_per_game.toFixed(1)}</p>
        </div>
        <div>
          <p className="text-[var(--text-muted)]">EPA/Play</p>
          <p className={`font-headline text-lg tabular-nums ${data.epa_per_play >= 0 ? 'text-[var(--color-positive)]' : 'text-[var(--color-negative)]'}`}>
            {data.epa_per_play.toFixed(3)}
          </p>
        </div>
        <div>
          <p className="text-[var(--text-muted)]">Avg Margin</p>
          <p className={`font-headline text-lg tabular-nums ${data.margin_per_game >= 0 ? 'text-[var(--color-positive)]' : 'text-[var(--color-negative)]'}`}>
