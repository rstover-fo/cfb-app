...
| 3 | `get_passing_charting` | MCP / eve / advisor / Discord | *requested* `api.passing_charting_player_season`, `_team_season` | both metric-specific denominators (`air_yards_attempts_available` AND `yards_after_catch_attempts_available`) in every payload -- air-yards and YAC charting cover different play sets, so one shared number misstates coverage for whichever metric it does not describe; default coverage floor stated in the description; NULL = not-yet-charted; 2025+ only; `parse_status='partial'` rows are provisional |
