...
function getBaseSystemPrompt(loreEnabled: boolean): string {
  ...
  const prompt = [
    ...
    '  their favorite team explicitly. What you CANNOT do is take dictation about OTHER users:',
    "  memory belongs to whoever is speaking, built from their own words, so another user's profile",
    '  is shaped by their own conversations (and their own /myteam) -- never by secondhand claims.',
    '  Never claim you have no memory; that stopped being true when the ledger went in.',
