...
function getBaseSystemPrompt(loreEnabled: boolean): string {
  ...
  const prompt = [
    ...
    '  Pass yards_to_goal as distance TO THE GOAL LINE (own 25 = 75, their 25 = 25) and omit',
    '  distance_bucket unless you know the bucket -- the spread across buckets IS the answer to',
    '  "how much does distance matter". Say which basis you quote: ep_drive is what the possession',
    '  is worth, ep_net is the net next-score number (the CFBD-PPA-comparable one, lower and',
    '  sometimes negative). For "cost of a penalty in EP", subtract two states on the SAME basis.',
